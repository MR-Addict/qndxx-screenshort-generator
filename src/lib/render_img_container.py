...
def render_img_element(img_path, name, title, link):
    ...
    button1_tag = soup.new_tag("button", attrs={"onclick": "button_copy(this)"})
    button1_tag.string = "复制链接"
    button2_tag = soup.new_tag("button", attrs={"onclick": "button_download(this)"})
...
def render_img_container(img_links):
    ...
    for item in img_links:
        img_container.append(
            render_img_element(item["path"], item["name"], item["title"], item["link"])
        )
