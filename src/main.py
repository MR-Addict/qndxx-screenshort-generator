...
if __name__ == "__main__":
    ...
    with open(
        os.path.join(os.getcwd(), "static/index.html"), "r", encoding="utf-8"
    ) as file:
        raw_html_str = file.read()
    formated_date = datetime.now(pytz.timezone("Asia/Shanghai")).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    ...
    with open(
        os.path.join(os.getcwd(), "public/index.html"), "w", encoding="utf-8"
    ) as output_html:
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        executor.map(
            lambda x: download_img(x["link"], "public/images/", x["title"] + ".jpg"),
            img_links,
        )
